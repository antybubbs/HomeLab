...
async def security_headers(request: Request, call_next):
    ...
    response.headers["Content-Security-Policy"] = f"default-src 'self'; connect-src 'self' {ws_scheme}://{request.url.netloc}; img-src 'self' data:; style-src 'self' 'unsafe-inline'; style-src-attr 'unsafe-inline'; script-src 'self'; worker-src 'self' blob:; object-src 'none'; base-uri 'self'; frame-ancestors *; form-action 'self'"
    if is_static_asset:
